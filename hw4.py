...
def sub_interval(x, y):
    ...
    """
    "*** YOUR CODE HERE ***"
    min_diff = lower_bound(x) - upper_bound(y)
    max_diff = upper_bound(x) - lower_bound(y)
    return interval(min_diff, max_diff)
...
def mul_interval_fast(x, y):
    ...
    if x0 >= 0 and y0 >= 0:
        return interval(x0*y0, x1*y1)
    elif x0 < 0 and y0 >= 0:
        if x1 >= 0:
            return interval(x0*y1, x1*y1)
        else:
            return interval(x0*y1, x1*y0)
    elif x0 >= 0 and y0 < 0:
        if y1 >= 0:
            return interval(x1*y0, x1*y1)
        else:
            return interval(x1*y0, x0*y1)
    else:
        if x1 >= 0 and y1 >= 0:
            return interval(min(x0*y1, x1*y0), max(x0*y0, x1*y1))
        elif x1 <0 and y1 >= 0:
            return interval(x0*y1, x0*y0)
        elif x1 >= 0 and y1 < 0:
            return interval(x1*y0, x0*y0)
        else:
            return interval(x1*y1, x0*y0)
...
def percent(x):
    ...
    "*** YOUR CODE HERE ***"
    average = (x[0] + x[1])/2
    return (x[1] - average)/average*100
...
def quadratic(x, a, b, c):
    ...
    inflection = -b/(2*a)
    if inflection < x[0] or inflection > x[1]:
        extreme = x
    else:
        extreme = [inflection]

    def expression(t):
        return [a*x*x + b*x + c for x in t]
    if a < 0:
        maximum = max(expression(extreme))
        minimum = min(expression(x))
    else:
        minimum = min(expression(extreme))
        maximum = max(expression(x))
...
def sum_nonzero_with_generator_reduce(seq):
    ...
    """
    "*** YOUR CODE HERE ***"
    elements = (square_interval for x in seq if non_zero(x))
    return reduce(add_interval, elements, zero)
